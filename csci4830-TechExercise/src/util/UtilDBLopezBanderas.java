...
public class UtilDBLopezBanderas {
	...
	public static List<Course> listCourses(String keyword) {
		...
		try {
			tx = session.beginTransaction();
			// System.out.println((Course)session.get(Course.class, 1)); // use "get" to
			// fetch data
	...
	public static boolean addCourse(String id, String course_id, String section, String semester, String title,
			String credit_hours, String grade) {
		Session session = getSessionFactory().openSession();
		Transaction tx = null;
		boolean result = true;
		try {
			tx = session.beginTransaction();
			session.save(new Course(Integer.valueOf(id), course_id, section, semester, title,
					Integer.valueOf(credit_hours), grade));
			tx.commit();
		} catch (HibernateException e) {
			result = false;
			if (tx != null)
	...
	public static boolean deleteCourse(String id, String course_id, String section, String semester) {
		...
		try {
			...
			Course course = (Course) session.createCriteria(Course.class)
					.add(Restrictions.eq("id", Integer.valueOf(id))).add(Restrictions.eq("course_id", course_id))
